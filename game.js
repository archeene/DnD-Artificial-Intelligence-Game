...
const DEFAULT_DM_PROMPT = `Do not mention venice or AI. Limit responses to 1 paragraph long. Each response should only describe a single NPC. If multiple NPCs are present at the scene, describe one with your response and prompt the user to ask you about the others. Strictly roleplay as an expert Dungeon Master and rules lawyer for Dungeons & Dragons 4th Edition. Always precisely follow the rules, factoring in the dice rolls and relevant bonuses and stats from the character sheets in the quest log. Do not break immersion of the game. Your job is to create compelling narratives and NPC characters while strictly following the game rules. All actions should be advancing the overall narrative which is specified by the user. Key SRD sections to prioritize: - Combat: Initiative, actions, bonus actions, reactions, attack rolls, damage, conditions (PHB Chapter 9) - Spellcasting: Spell slots, casting time, components, concentration, saving throws (PHB Chapter 10) - Ability Checks: Advantage/disadvantage, proficiency, DC setting - Classes/Races/Backgrounds: Use only SRD content (core races/classes only — no subclasses beyond SRD without user input) - Monsters: Use SRD stat blocks exactly - Magic Items: SRD only Never make up rules. If unsure, say "According to the SRD..." and provide the most accurate interpretation. For quick lookups, respond with the rule first, then explain in context.

CRITICAL FORMATTING RULE FOR ALL NPCS AND MONSTERS:
Whenever you introduce a monster or named NPC, you MUST use this EXACT format with NO DEVIATIONS:

=== STAT BLOCK: [Exact Name] ===
Level: [number] [role] ([source])
HP: [current]; Bloodied [value]
AC: [number]
Fortitude: [number], Reflex: [number], Will: [number]
Speed: [number]
Initiative: [modifier]
Str [score], Con [score], Dex [score], Int [score], Wis [score], Cha [score]
Skills: [list with bonuses]
[Other relevant traits or powers in short bullet points]
=== END STAT BLOCK ===

NPC DESCRIPTION: [Detailed visual physical description of the NPC appearance, clothing, demeanor - this will be used to generate character art]

The "NPC DESCRIPTION:" line MUST come AFTER the stat block and MUST start with exactly "NPC DESCRIPTION:" followed by the visual description. Do not deviate from this format.`;
...
let meetCharacterIndex = 0; // Tracks which NPC to generate next

// TTS (Text-to-Speech) settings
let ttsEnabled = localStorage.getItem('ttsEnabled') === 'true' || false;
let ttsVoiceIndex = parseInt(localStorage.getItem('ttsVoiceIndex')) || 0;
...
function loadStateFromData(data) {
  ...
  // Restore background map if saved
  if (data.mapImageSrc !== undefined && data.mapImageSrc !== null && sceneAssets[data.mapImageSrc]) {
...
function saveCharacterSheets() {
  ...
  // Sync character sheets in multiplayer
  if (typeof syncCharacterSheets === 'function') syncCharacterSheets();
}
...
document.addEventListener('keydown', (e) => {
  ...
  if (dx !== 0 || dy !== 0) {
    ...
    if (!overlaps) {
      ...
      selectedUnit.y = newY;
      draw();
      // Sync unit positions in multiplayer
      if (typeof syncUnits === 'function') syncUnits();
    }
  }
});
...
function showCharacterDetailsPanel(unit) {
  ...
  saveSheetBtn.onclick = () => {
    ...
    if (unit.sheet.exp) sheetText += `Experience: ${unit.sheet.exp}\n`;
    if (unit.sheet.nextLevel) sheetText += `Next Level: ${unit.sheet.nextLevel}\n`;
    ...
    questLog += sheetText;
    localStorage.setItem(QUEST_LOG_KEY, questLog);
    if (typeof syncQuestLog === 'function') syncQuestLog();
  ...
  // Experience and Next Level row (directly below size buttons)
  const expRow = document.createElement('div');
  expRow.style.display = 'flex';
  expRow.style.gap = '15px';
  expRow.style.marginBottom = '20px';
  expRow.style.justifyContent = 'center';

  const expWrapper = document.createElement('div');
  expWrapper.style.flex = '1';
  expWrapper.style.maxWidth = '45%';
  ...
  expWrapper.appendChild(expLabel);

  const expInput = document.createElement('input');
  expInput.type = 'text';
  expInput.value = (unit.sheet && unit.sheet.exp) || '';
  expInput.style.width = '100%';
  expInput.style.padding = '8px';
  expInput.style.background = '#222';
  expInput.style.color = '#0f0';
  expInput.style.border = '1px solid #0f0';
  expInput.style.borderRadius = '5px';
  expInput.style.boxSizing = 'border-box';
  expInput.onchange = () => {
    if (!unit.sheet) unit.sheet = {};
    unit.sheet.exp = expInput.value;
    draw();
  };
  expWrapper.appendChild(expInput);
  expRow.appendChild(expWrapper);

  const nextWrapper = document.createElement('div');
  nextWrapper.style.flex = '1';
  nextWrapper.style.maxWidth = '45%';
  ...
  nextInput.type = 'text';
  nextInput.value = (unit.sheet && unit.sheet.nextLevel) || '';
  nextInput.style.width = '100%';
  nextInput.style.padding = '8px';
  nextInput.style.background = '#222';
  nextInput.style.color = '#0f0';
  nextInput.style.border = '1px solid #0f0';
  nextInput.style.borderRadius = '5px';
  nextInput.style.boxSizing = 'border-box';
  nextInput.onchange = () => {
    if (!unit.sheet) unit.sheet = {};
    unit.sheet.nextLevel = nextInput.value;
    draw();
  };
  nextWrapper.appendChild(nextInput);
  expRow.appendChild(nextWrapper);
  ...
  container.appendChild(createHorizontalPair('Level + Class', 'class', 'Race', 'race'));
  container.appendChild(createHorizontalPair('HP', 'hp', 'AC', 'ac'));

  // Attack Bonus header
  const attackHeader = document.createElement('h3');
  attackHeader.textContent = 'Attack Bonus';
  attackHeader.style.textAlign = 'center';
  attackHeader.style.margin = '20px 0 10px 0';
  attackHeader.style.fontSize = '16px';
  container.appendChild(attackHeader);

  // Melee and Ranged inputs
  const attackRow = createHorizontalPair('Melee', 'melee', 'Ranged', 'ranged');
  ...
  // Duplicate Save button at the very bottom
  const saveSheetBtnBottom = saveSheetBtn.cloneNode(true);
  saveSheetBtnBottom.onclick = saveSheetBtn.onclick;
  saveSheetBtnBottom.style.marginTop = '20px';
  saveSheetBtnBottom.style.marginBottom = '0';
  container.appendChild(saveSheetBtnBottom);
...
function createChatBox() {
  ...
  ttsBtn.id = 'ttsSpeakerBtn';
  ttsBtn.textContent = '🔇 TTS';
  ttsBtn.style.padding = '6px 12px';
  ttsBtn.style.background = '#333';
  ttsBtn.style.color = '#0f0';
  ttsBtn.style.border = '2px solid #0f0';
  ttsBtn.style.borderRadius = '6px';
  ttsBtn.style.cursor = 'pointer';
  ttsBtn.title = 'Toggle Text-to-Speech for DM messages';
  modeRow.appendChild(ttsBtn);

  // Voice Chat Button
  const voiceChatBtn = document.createElement('button');
  voiceChatBtn.id = 'voiceChatBtn';
  voiceChatBtn.textContent = '🎤 Voice';
  voiceChatBtn.style.padding = '6px 12px';
  voiceChatBtn.style.background = '#333';
  voiceChatBtn.style.color = '#0f0';
  voiceChatBtn.style.border = '2px solid #0f0';
  voiceChatBtn.style.borderRadius = '6px';
  voiceChatBtn.style.cursor = 'pointer';
  voiceChatBtn.title = 'Toggle Voice Chat (Multiplayer)';
  modeRow.appendChild(voiceChatBtn);
  ...
  meetCharacterBtn.onclick = () => {
    ...
    // Try to find "NPC DESCRIPTION:" first (new format)
    const npcDescIndex = lastDmText.indexOf('NPC DESCRIPTION:');
    if (npcDescIndex !== -1) {
      descriptionText = lastDmText.substring(npcDescIndex + 16).trim(); // Skip "NPC DESCRIPTION:"
    } else {
      // Fallback 1: Look for old "NPC:" format
      const npcIndex = lastDmText.toLowerCase().indexOf('npc:');
      if (npcIndex !== -1) {
        descriptionText = lastDmText.substring(npcIndex + 4).trim(); // Skip "NPC:"
      } else {
        // Fallback 2: Try to extract description from stat block name
        const statBlockRegex = /=== STAT BLOCK: (.*?) ===/;
        const match = lastDmText.match(statBlockRegex);
        if (match) {
          characterName = match[1].trim();
          // Look for any descriptive text after END STAT BLOCK
          const endBlockIndex = lastDmText.indexOf('=== END STAT BLOCK ===');
          if (endBlockIndex !== -1) {
            const afterBlock = lastDmText.substring(endBlockIndex + 23).trim();
            if (afterBlock.length > 10) {
              descriptionText = afterBlock;
            } else {
              // Use character name as description
              descriptionText = characterName;
            }
          } else {
            descriptionText = characterName;
          }
        } else {
          // Fallback 3: Use entire message as description
          alertStyled('No NPC description format found. Using entire DM message.');
          descriptionText = lastDmText.substring(0, 200); // First 200 chars
        }
      }
    }

    if (descriptionText.length === 0) {
      alertStyled('Could not extract NPC description from DM message.');
      return;
    }

    // Truncate if too long (max 500 chars for image generation)
    if (descriptionText.length > 500) {
      descriptionText = descriptionText.substring(0, 500);
    }

    // Build full prompt from the description
    const charPrompt = `${descriptionText}. Full body pixel art portrait in 16-bit retro style.`;

    // Open Character Generator panel
    const charGenBtn = document.getElementById('btnCharacterGen');
    if (charGenBtn) charGenBtn.click();

    // Inject prompt and generate
    setTimeout(() => {
      ...
      if (charInput) {
        ...
        // Wait for image to be generated and auto-process it
        let attempts = 0;
        const maxAttempts = 120; // 60 seconds - enough time for long generation
        const checkInterval = setInterval(() => {
          attempts++;
          const imgs = document.querySelectorAll('#characterMessages img');
            ...
            if (imgs.length > 0) {
              ...
              if (latestImg.src && latestImg.src.startsWith('data:image')) {
                ...
                const newUnit = {
                  ...
                  y: Math.floor(GRID_SIZE / 2),
                  type: 'character',
                  spriteSrc: latestImg.src,
                  sheet: {},
                  size: 1
                };
                units.push(newUnit);

                // === AUTO-POPULATE SHEET FROM LATEST DM STAT BLOCK ===
                const chatDivs = document.querySelectorAll('#chatMessages > div');
                let lastDmText = '';
                for (let i = chatDivs.length - 1; i >= 0; i--) {
                  const div = chatDivs[i];
                ...
                if (lastDmText) {
                  console.log('Meet Character: Searching for stat block in DM text...');
                  const statBlockRegex = /=== STAT BLOCK: (.*?) ===\s*([\s\S]*?)\s*=== END STAT BLOCK ===/;
                  const match = lastDmText.match(statBlockRegex);
                  if (match) {
                    console.log('Meet Character: Stat block found!');
                    const name = match[1].trim();
                    const block = match[2].trim();
                    const blockLines = block.split('\n').map(l => l.trim());
                    console.log('Stat block lines:', blockLines);

                    newUnit.sheet.name = name;

                    // Level + Class
                    const levelLine = blockLines[0];
                    const levelMatch = levelLine.match(/Level:\s*(\d+)\s*(.*?)\s*\(/);
                    if (levelMatch) {
                      newUnit.sheet.class = `Level ${levelMatch[1]} ${levelMatch[2].trim()}`;
                    }

                    // HP - bloodied ×2
                    const hpLine = blockLines.find(l => l.startsWith('HP:'));
                    if (hpLine) {
                      const bloodiedMatch = hpLine.match(/Bloodied\s*(\d+)/i);
                      if (bloodiedMatch) {
                        const max = parseInt(bloodiedMatch[1]) * 2;
                        newUnit.sheet.hp = `${max}/${max}`;
                      }
                    }

                    // AC
                    const acLine = blockLines.find(l => l.startsWith('AC:'));
                    if (acLine) {
                      const acMatch = acLine.match(/AC:\s*(\d+)/i);
                      if (acMatch) newUnit.sheet.ac = acMatch[1];
                    }

                    // Abilities
                    const abLine = blockLines.find(l => /Str\s*\d+/.test(l));
                    if (abLine) {
                      const scores = abLine.split(',').map(s => s.trim().match(/(\d+)/)?.[1]);
                      if (scores.length === 6) {
                        newUnit.sheet.str = scores[0];
                        newUnit.sheet.con = scores[1];
                        newUnit.sheet.dex = scores[2];
                        newUnit.sheet.int = scores[3];
                        newUnit.sheet.wis = scores[4];
                        newUnit.sheet.cha = scores[5];
                      }
                    }

                    // Skills
                    const skillsLine = blockLines.find(l => l.startsWith('Skills:'));
                    if (skillsLine) {
                      newUnit.sheet.skills = skillsLine.replace(/^Skills:\s*/i, '').trim();
                    }

                    // Equipment
                    const equipLine = blockLines.find(l => l.startsWith('Trained in arms:'));
                    if (equipLine) {
                      newUnit.sheet.equipment = equipLine.replace(/^Trained in arms:\s*/i, '').trim();
                    }

                    // === PERSIST THE POPULATED SHEET ===
                    characterSheets[newUnit.spriteSrc] = newUnit.sheet;
                    saveCharacterSheets();
                    console.log('Character sheet saved:', newUnit.sheet);

                    alertStyled(`Character "${name}" added to map with stats!`);
                  } else {
                    console.log('Meet Character: No stat block found in DM message');
                    alertStyled('Character added to map (no stat block found).');
                  }
                } else {
                  console.log('Meet Character: No DM text found');
                }
            ...
            if (attempts >= maxAttempts) {
              clearInterval(checkInterval);
              // Silent timeout - don't show error, user can try manually if needed
            }
          }, 500);
      }
    }, 400);
...
function speakText(text) {
  ...
  if (!cleanText) return; // Don't speak if nothing left
  ...
  if (voices.length > 0) {
    // Try to find a more natural voice (neural/enhanced voices often have these keywords)
    ...
    ) || voices[ttsVoiceIndex % voices.length];
...
function toggleTTS() {
  ...
  const ttsBtn = document.getElementById('ttsSpeakerBtn');
  if (ttsBtn) {
    if (ttsEnabled) {
      ttsBtn.textContent = '🔊 TTS';
      ttsBtn.style.background = '#0f0';
      ttsBtn.style.color = '#000';
      alertStyled('Text-to-Speech enabled');
    } else {
      ttsBtn.textContent = '🔇 TTS';
      ttsBtn.style.background = '#333';
      ttsBtn.style.color = '#0f0';
      window.speechSynthesis.cancel(); // Stop any ongoing speech
      alertStyled('Text-to-Speech disabled');
    }
  }
}
...
async function toggleVoiceChat() {
  if (!isMultiplayerConnected) {
    alertStyled('Voice chat requires multiplayer connection');
    return;
  }
  ...
  if (!voiceChatEnabled) {
    try {
      ...
      processor.onaudioprocess = (e) => {
        ...
        if (socket && isMultiplayerConnected) {
          socket.emit('voice-data', Array.from(audioData));
        }
      };

      // Update button
      if (voiceChatBtn) {
        voiceChatBtn.textContent = '🎙️ Voice';
        voiceChatBtn.style.background = '#f00'; // Red when active/recording
        voiceChatBtn.style.color = '#fff';
      }
      alertStyled('Voice chat enabled - you are live!');
    } catch (err) {
      console.error('Microphone access denied:', err);
      alertStyled('Microphone access denied. Check browser permissions.');
    ...
    alertStyled('Voice chat disabled');
  }
}

// Initialize TTS and voice chat button handlers when chat is created
window.addEventListener('load', () => {
  setTimeout(() => {
    ...
    const voiceChatBtn = document.getElementById('voiceChatBtn');
    if (voiceChatBtn) {
      voiceChatBtn.onclick = toggleVoiceChat;
    }
  }, 100);
});
...
async function sendChatMessage() {
  ...
  if (questLog.trim()) {
    messages.push({
      role: 'system',
      content: `=== QUEST HISTORY SO FAR ===\n${questLog.trim()}\n=== END QUEST HISTORY ===\nUse this information to maintain perfect narrative continuity and reference past events naturally when relevant. Never reveal any future plot points.`
    });
  }
  ...
  try {
    ...
    // === HP COMMAND PARSER (now checks BOTH user message AND AI reply) ===
    const processHpCommand = (text, source = 'user') => {
      const hpCommandRegex = /^(?:\[([^[\]]+)\]|\b([A-Z]?[a-z]+)\b)\s*(?:takes\s+(\d+)\s*(?:damage|dmg)|heals?\s+(\d+)|is\s+healed\s+for\s+(\d+)|sets?\s+HP\s+to\s+(.+))/i;
      const match = text.match(hpCommandRegex);
      if (match) {
        const charName = (match[1] || match[2]).trim();
        const targetUnit = units.find(u => 
          u.sheet && 
          u.sheet.name && 
          u.sheet.name.trim().toLowerCase() === charName.toLowerCase()
        );

        if (!targetUnit || !targetUnit.sheet) {
          if (source === 'assistant') {
            addChatMessage('assistant', `(System: No character named "${charName}" found for auto-HP update.)`);
          }
        } else {
          let currentHP = 0;
          let maxHP = null;
          const hpText = (targetUnit.sheet.hp || '').trim();
          const existing = hpText.match(/^(\d+)\/?(\d*)/);
          if (existing) {
            currentHP = parseInt(existing[1]) || 0;
            if (existing[2]) maxHP = parseInt(existing[2]);
          }

          let newCurrent = currentHP;

          if (match[3]) { // takes X damage/dmg
            const dmg = parseInt(match[3]);
            newCurrent = Math.max(0, currentHP - dmg);
          } else if (match[4] || match[5]) { // heals
            const heal = parseInt(match[4] || match[5]);
            const cap = maxHP !== null ? maxHP : currentHP;
            newCurrent = Math.min(cap, currentHP + heal);
          } else if (match[6]) { // sets HP to ...
            const setStr = match[6].trim();
            const setMatch = setStr.match(/^(\d+)(?:\/(\d+))?$/);
            if (setMatch) {
              newCurrent = parseInt(setMatch[1]);
              if (setMatch[2]) maxHP = parseInt(setMatch[2]);
            } else {
              newCurrent = parseInt(setStr) || 0;
            }
            newCurrent = Math.max(0, newCurrent);
          }

          const newHpString = maxHP !== null ? `${newCurrent}/${maxHP}` : `${newCurrent}`;
          targetUnit.sheet.hp = newHpString;

          characterSheets[targetUnit.spriteSrc] = targetUnit.sheet;
          saveCharacterSheets();

          draw();

          if (source === 'assistant') {
            addChatMessage('assistant', `(System: Auto-updated ${targetUnit.sheet.name} HP → ${newHpString})`);
          } else {
            addChatMessage('assistant', `(System: ${targetUnit.sheet.name} HP updated → ${newHpString})`);
          }
        }
      }
    };

    // Process user message (existing behavior)
    processHpCommand(message, 'user');

    // Process AI reply (new behavior)
    processHpCommand(reply, 'assistant');
...
function addChatMessage(role, content, skipBroadcast = false) {
  ...
  // Speak DM/assistant messages with TTS if enabled
  if (role === 'assistant' && ttsEnabled) {
    speakText(content);
  }

  // Broadcast to other players (avoid infinite loop with skipBroadcast flag)
  if (isMultiplayerConnected && socket && !skipBroadcast) {
    socket.emit('chat-message', { role, content });
  }
}
...
function createSceneGenPanel() {
  ...
  // === Log Background Scene button ===
  const logSceneBtn = document.createElement('button');
  logSceneBtn.textContent = 'Log Background Scene';
  logSceneBtn.style.width = '100%';
  logSceneBtn.style.margin = '10px 0';
  logSceneBtn.style.background = '#004'; // Dark blue to match Log Chat
  logSceneBtn.style.color = '#0f0';
  logSceneBtn.style.border = '2px solid #0f0';
  logSceneBtn.style.borderRadius = '6px';
  logSceneBtn.style.cursor = 'pointer';
  logSceneBtn.style.padding = '8px';
  logSceneBtn.onclick = () => {
    // Small delay to match "Enter Scene" timing and ensure canvas is rendered
    setTimeout(() => {
      analyzeSceneWithVision();
    }, 800);
  };
  container.appendChild(logSceneBtn);
...
function setupSidebar() {
  const map = {
    ...
    btnMultiplayer: 'multiplayerPanel'
  };
...
function startQuestGeneration() {
  // Clear previous DM Secrets immediately
  dmSecrets = '';
  localStorage.setItem(DM_SECRETS_KEY, '');
...
2. Briefly describe the party members (class, race, key backstory elements).
3. What tone do you prefer? Options: Heroic, Gritty, Comedic, Horror, Intrigue.
I will generate the complete campaign (1 minute)`);
  ...
  addChatMessage = function(role, content) {
    ...
    if (role === 'assistant' && content.includes('═══════════════════════════════════════════') && content.includes('A Three-Act Adventure for')) {
      ...
      if (typeof syncDMSecrets === 'function') syncDMSecrets();
      ...
      textarea.value = `IMPORTANT: SHORT RESPONSES. 8 Sentences max per visible response. Strictly roleplay as an expert Dungeon Master and rules lawyer for Dungeons & Dragons 4th Edition. For any rules question, quote or paraphrase the exact rule from the 4th SRD. Your job is to create compelling narratives and NPC characters while strictly following the game and prompt formatting rules to the letter. All actions should be advancing the overall quest narrative which is specified in the DM secrets file. Key SRD sections to prioritize: - Combat: Initiative, actions, bonus actions, reactions, attack rolls, damage, conditions - Spellcasting: Spell slots, saving throws - Ability Checks: Advantage/disadvantage, proficiency, DC setting - Classes/Races/Backgrounds: Use only SRD content (core races/classes only — no subclasses beyond SRD without user input) - Monsters: Use SRD stat blocks exactly - Magic Items: SRD only Never make up rules.`;
      ...
      setTimeout(() => {
        const input = document.getElementById('chatInput');
        input.value = '[START ADVENTURE] Begin the game now by describing a map of the first scene from Act I using only currently visible information.';
        sendChatMessage();
      }, 800);
    }
  };
...
function setupMultiplayerListeners() {
  ...
  // Receive chat message from other player
  socket.on('chat-message', (data) => {
    addChatMessage(data.role, data.content, true); // skipBroadcast = true to avoid loop
  });

  // Receive game state update from other player
  socket.on('game-state-update', (data) => {
    applyGameState(data);
  });

  // Receive voice data from other player
  socket.on('voice-data', (audioData) => {
    if (!voiceChatEnabled) return; // Only play if voice chat is enabled
...
function broadcastGameState() {
  if (!socket || !isMultiplayerConnected) return;

  const gameState = {
    units: units,
    customMapImageSrc: customMapImageSrc,
    ...
    npcPromptAssets: npcPromptAssets
  };
...
function applyGameState(state) {
  ...
  if (state.customMapImageSrc !== undefined) {
    customMapImageSrc = state.customMapImageSrc;
    if (customMapImageSrc) {
      customMapImage = new Image();
      customMapImage.src = customMapImageSrc;
      customMapImage.onload = () => draw();
    } else {
  ...
  if (state.characterSheets !== undefined) {
    characterSheets = state.characterSheets;
  }
  if (state.dmSecrets !== undefined) {
    dmSecrets = state.dmSecrets;
    localStorage.setItem(DM_SECRETS_KEY, dmSecrets);
  }
  if (state.questLog !== undefined) {
    questLog = state.questLog;
    localStorage.setItem(QUEST_LOG_KEY, questLog);
  }
  if (state.chatHistory !== undefined) {
  ...
  if (state.currentChatMode !== undefined) {
    currentChatMode = state.currentChatMode;
  }
  if (state.characterAssets !== undefined) {
    characterAssets = state.characterAssets;
    localStorage.setItem(CHARACTER_STORAGE_KEY, JSON.stringify(characterAssets));
    populateLeftBox();
  }
  if (state.sceneAssets !== undefined) {
    sceneAssets = state.sceneAssets;
    localStorage.setItem(SCENE_STORAGE_KEY, JSON.stringify(sceneAssets));
    populateLeftBox();
  }
  if (state.npcPromptAssets !== undefined) {
    npcPromptAssets = state.npcPromptAssets;
    localStorage.setItem(NPC_PROMPTS_KEY, JSON.stringify(npcPromptAssets));
    populateLeftBox();
  }
}

// Add sync helpers for specific game actions
function syncUnits() {
  if (isMultiplayerConnected && socket) {
    socket.emit('game-state-update', { units: units });
  }
}

function syncCharacterSheets() {
  if (isMultiplayerConnected && socket) {
    socket.emit('game-state-update', { characterSheets: characterSheets });
  }
}

function syncMap() {
  if (isMultiplayerConnected && socket) {
    socket.emit('game-state-update', { customMapImageSrc: customMapImageSrc });
  }
}

function syncQuestLog() {
  if (isMultiplayerConnected && socket) {
    socket.emit('game-state-update', { questLog: questLog });
  }
}

function syncDMSecrets() {
  if (isMultiplayerConnected && socket) {
    socket.emit('game-state-update', { dmSecrets: dmSecrets });
  }
}
